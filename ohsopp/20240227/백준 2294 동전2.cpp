...
(1)의 경우는 동전의 개수에 제약이 없으므로 오름차순 탐색으로 dp를 설계한다.
(2)의 경우는 동전이 1개만 사용되어야 하므로 내림차순 탐색으로 dp를 설계한다. (오름차순 탐색 방식으로도 구현가능하지만 dp를 2차원 배열로 사용해야함 -> 메모리 낭비)

(2)의 경우를 오름차순 탐색으로 구현한다면,
...
int main(){
    ...
    while (n--) {
        cin >> c;
        for(int i = c; i <= k; i++)    // 동전 개수에 제약이 없으므로 오름차순 탐색
            d[i] = min(d[i], d[i - c] + 1);
    }
    cout << (d[k] < 1e5 ? d[k] : -1);
}
