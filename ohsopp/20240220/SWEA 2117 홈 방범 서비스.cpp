...
(5) k를 2*n부터 1까지 탐색하여 (현재 ans) >= (k일 때 최대로 가능한 서비스 영역) 이라면 서비스 영역이 모두 집이더라도 ans를 넘을 수 없으므로 탐색 중지 -> 98ms
...
int main() {
	...
	for (int tc = 1; tc <= t; tc++) {
		...
		for (int i = 1; i <= n; i++)
			for (int j = 1; j <= n; j++) {
				cin >> cnt;
				if (cnt) home.push_back({ i,j });	// 집 위치만 저장
			}
		/*--------------------- 여기까지는 입력 ------------------------*/
		
		ans = 0;
		for (int k = 2 * n; k >= 1; k--) {
			if (k * k + (k - 1) * (k - 1) <= ans) break;	// 현재 ans가 이미 k일 때의 서비스 영역보다 같거나 크다면 break
			ofs = (k - 1) / 2;	// 탐색 시간 단축을 위해 대각선 방향으로 (1,1)과 (n,n)이 이미 포함되는 offset 거리 계산
